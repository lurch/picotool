...
std::array<std::array<string, 30>, 10> pin_functions{{
    {"", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", ""},
    {"SPI0 RX", "SPI0 CSn", "SPI0 SCK", "SPI0 TX", "SPI0 RX", "SPI0 CSn", "SPI0 SCK", "SPI0 TX", "SPI1 RX", "SPI1 CSn", "SPI1 SCK", "SPI1 TX", "SPI1 RX", "SPI1 CSn", "SPI1 SCK", "PI1 TX", "SPI0 RX", "SPI0 CSn", "SPI0 SCK", "SPI0 TX", "SPI0 RX", "SPI0 CSn", "SPI0 SCK", "SPI0 TX", "SPI1 RX", "SPI1 CSn", "SPI1 SCK", "SPI1 TX", "SPI1 RX", "SPI1 CSn"},
...
auto device_selection =
    (
        ...
        option('f', "--force").set(settings.force) % "Force a device not in BOOTSEL mode but running compatible code to reset so the command can be executed. Unless the command itself causes a reboot, the device will be rebooted back to application mode" +
                option('F', "--force-no-reboot").set(settings.no_reboot_if_forced) % "Force a device not in BOOTSEL mode but running compatible code to reset so the command can be executed. Unless the command itslef causes a reboot, the device will be left connected and accessible to picotool, but without the RPI-RP2 drive mounted"
...
void load_command::execute(device_map &devices) {
    ...
    if (settings.load.execute) {
        ...
        settings.no_reboot_if_forced = true; // no point rebooting again!
    }
}
...
int main(int argc, char **argv) {
    ...
    if (settings.no_reboot_if_forced) settings.force = true;
                ...
                if (tries) {
                    if (settings.no_reboot_if_forced) {
                        ...
                        if (devices[dr_vidpid_bootrom_ok].size() == 1) {
                            ...
                            std::cout << "\nThe device was rebooted back into application mode.\n";
                        }
                    }
                }
                break;
